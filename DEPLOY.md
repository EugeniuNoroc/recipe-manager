...
| Поведение | Отключение действует до явного нажатия «Включить обратно» |
| Напоминание | На каждой странице сайта — красный sticky-баннер с кнопкой «Включить обратно» |
...
3. Показать: красный баннер сверху, сессия сброшена, избранное недоступно, `/stats.php` отдаёт нули
4. Нажать «Включить обратно» (в баннере или в панели) → всё восстановилось
5. «Отключить MySQL» → показать страницу maintenance (HTTP 503)
6. «Включить обратно» → сайт снова работает
7. **После демонстрации обязательно нажать «Включить обратно» — флаги не сбрасываются автоматически**
...
Если флаги не сброшены через панель — сбрось вручную:
```bash
# (lab)
bash /home/lab/app/deploy/emergency-reset.sh
```
...
## Аварийный сброс chaos-флагов

Если потерян доступ к `/admin/chaos.php`, забыт пароль или просто нужно быстро
восстановить сайт — сбрось флаги напрямую через SSH:

```bash
# Вариант A — готовый скрипт (запускать под пользователем lab)
ssh lab@178.104.227.19
bash /home/lab/app/deploy/emergency-reset.sh
```

```bash
# Вариант B — однострочник под root
ssh root@178.104.227.19
echo '{"redis_disabled":false,"mysql_disabled":false}' > /home/lab/app/storage/chaos.json
chown lab:lab /home/lab/app/storage/chaos.json
```

Сайт восстановится на следующем запросе — перезапуск сервиса не нужен.
`ChaosFlags` читает файл при каждом HTTP-запросе.
