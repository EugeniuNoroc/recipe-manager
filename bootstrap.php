...
/**
 * Загрузчик приложения (bootstrap).
 *
 * Инициализирует все сервисы и делает их доступными как переменные в скриптах public/.
 * Порядок загрузки критичен: ChaosFlags → Redis → MySQL → Services → CSRF → Guard.
 * При падении MySQL показывает страницу обслуживания (HTTP 503) вместо белого экрана.
 * Заблокированные пользователи принудительно разлогиниваются на каждом запросе.
 */
