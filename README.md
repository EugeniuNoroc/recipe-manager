...
Встроенная система симуляции сбоев для демонстрации graceful degradation вживую. Преподаватель открывает панель, нажимает одну кнопку — сайт переходит в degraded-режим; ещё раз — всё восстанавливается. **Автовосстановление через 30 секунд** защищает от человеческого фактора.
