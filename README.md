...
Сетка карточек рецептов (1–3 колонки в зависимости от ширины экрана). Каждая карточка содержит
название, автора, время приготовления, дату, бейджи категории и сложности, счётчик просмотров.
В шапке — строка полнотекстового поиска по названию и ингредиентам и выпадающий фильтр по
категории. Кнопки «Редактировать» / «Удалить» отображаются только владельцу рецепта или
администратору.

### Административная панель (`/admin/index.php`)

Дашборд с четырьмя карточками статистики (пользователи, рецепты, категории, активные сессии
Redis). Ниже — быстрые ссылки на разделы: Пользователи, Рецепты, Категории, Теги, Статистика,
Chaos Panel. Боковое меню присутствует на всех admin-страницах через `templates/admin_layout.php`.

### Статистика просмотров (`/stats.php`)

Топ-10 рецептов по просмотрам, полученный из Redis Sorted Set `popular:recipes` через
`ZREVRANGE`. Доступна без авторизации. Каждая строка содержит название рецепта, автора и
количество просмотров.

### Chaos Engineering Panel (`/admin/chaos.php`)

Две кнопки-тумблера: «Отключить Redis» и «Отключить MySQL». Состояние хранится в
`storage/chaos.json`. При отключённом Redis на всех страницах появляется красный баннер
«⚠️ DEMO MODE» с автообновлением каждые 10 секунд. При отключённом MySQL сайт переходит
в режим обслуживания (HTTP 503) и восстанавливается без перезапуска сервера.
